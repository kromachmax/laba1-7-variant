...
int main(void) {
	...
	printf("Основной пункт.\n");
	printList(startPtr);
	int F;
	printf("Введите длину слова и вам выведутся все слова больше этого слова.");
	scanf_s("%d", &F);
	first(startPtr, F);
	int S;
	printf("Введите длину слова и вам выведутся все слова такой же длины .");
	scanf_s("%d", &S);
	second(startPtr, S);
	return 0;
}
